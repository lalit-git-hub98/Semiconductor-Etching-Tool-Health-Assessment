...
clear; clc; close all;
load('Baseline2DataSet.mat');
load('TestCase2DataSet.mat');

%% Develop Feature Matrices
% Build Training Feature Matrix
for k=1:length(Baseline2Run)
    s4Data = Baseline2Run{k}.Data(Baseline2Run{k}.Data(:, 2)==4, :);
    s5Data = Baseline2Run{k}.Data(Baseline2Run{k}.Data(:, 2)==5, :);
...
for k=1:length(TestCase2Run)
    s4Data_T = TestCase2Run{k}.Data(TestCase2Run{k}.Data(:, 2)==4, :);
    s5Data_T = TestCase2Run{k}.Data(TestCase2Run{k}.Data(:, 2)==5, :);
...
hold on
plot(SPE_alpha)
legend('SPE Testing Data','SPE Threshold', 'Location', 'northwest')
xlabel('Etcher run');
ylabel('SPE Value');
title('SPE Health Assessment Plot Etcher 2');
hold off
...
hold on
plot(t_sr_limit)
legend('T^2 Testing Data','T^2 Threshold', 'Location', 'northwest')
xlabel('Etcher run');
ylabel('T^2 Value');
title('T^2 Health Assessment Plot Etcher 2');
hold off
...
xlabel('Etcher run');
ylabel('MQE Value');
title('MQE Health Assessment Plot Etcher 2');
...
if isfile('Etcher2Health.xlsx')
    delete Etcher2Health.xlsx;
end
writetable(testingResults, 'Etcher2Health.xlsx');
